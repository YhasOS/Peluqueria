...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!booking) return res.status(403).json({ error: 'Solo puedes cancelar tus propias citas' });
  ...
  status: 'cancelled',
    ...
    return res.status(500).json({ error: 'Error cancelando cita', detail: error?.message || String(error) });
  }
}
