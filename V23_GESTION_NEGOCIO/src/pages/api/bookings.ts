...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      ...
      const bookings = await prisma.booking.findMany({
        ...
        include: {
          service: { select: { id: true, name: true, price: true, totalDuration: true } },
          bookingServices: { orderBy: { position: 'asc' }, include: { service: { select: { id: true, name: true } } } },
  ...
  url: '/staff',
      ...
      await prisma.bookingService.upsert({ where: { bookingId_serviceId: { bookingId: booking.id, serviceId: booking.serviceId } }, update: {}, create: { bookingId: booking.id, serviceId: booking.serviceId, position: 0, price: Number(booking.service.price || 0), duration: Number(booking.service.totalDuration || 0) } });
