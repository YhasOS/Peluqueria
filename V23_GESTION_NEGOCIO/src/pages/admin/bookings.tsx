...
interface Booking {
  ...
  bookingServices?: { service: { name: string }; price: number }[];
                ...
                {bookings.map((booking) => (
                  <tr key={booking.id} className="border-b last:border-0">
                    ...
                    <td className="px-3 py-3">
                      ...
                      <div className="text-xs text-gray-500">{booking.clientEmail || 'Sin email'}</div>
                    </td>
                    <td className="px-3 py-3">{booking.bookingServices?.length ? booking.bookingServices.map(s => s.service.name).join(' + ') : booking.service.name}</td>
